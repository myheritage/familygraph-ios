...
static NSString* kUserAgent = @"FamilyGraphConnect";
...
@interface FGRequest ()
@property (nonatomic,readwrite) FGRequestState state;
@end

@implementation FGRequest
...
+ (FGRequest *)getRequestWithParams:(NSMutableDictionary *) params
  ...
  FGRequest* request = [[[FGRequest alloc] init] autorelease];
...
- (id)formError:(NSInteger)code userInfo:(NSDictionary *) errorData {
   return [NSError errorWithDomain:@"familygraphErrDomain" code:code userInfo:errorData];
...
- (void)failWithError:(NSError *)error {
  ...
  self.state = kFGRequestStateError;
}
...
- (void)connect {
  ...
  self.state = kFGRequestStateLoading;
}
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
  ...
  if (self.state != kFGRequestStateError) {
    self.state = kFGRequestStateComplete;
  }
}
